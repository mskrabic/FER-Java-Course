...
public class LinkedListIndexedCollection<E> implements List<E> {
	...
	public int indexOf(Object value) {
		...
		int index = 0;
		for (ListNode<E> node = this.first; node != null; node = node.next, index++) {
			if (node.value.equals(value)) {
				return index;
			}
		}
	...
	public void remove(int index) {
		if (this.isEmpty())
			throw new IndexOutOfBoundsException("Collection is empty, nothing to remove!");
		if (index < 0 || index >= this.size) 
			throw new IndexOutOfBoundsException("Index should be between 0 and " + (this.size-1) + ",and was " + index + ".");
